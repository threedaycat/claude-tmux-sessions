...
- `o` 打开总览:一屏回答"我回来了,现在是什么局面"。**第一行就是结论** —— `5 个有结果等你看`
  ...
  今日 token 和近 14 天曲线钉在最下面,走卡片的时候一直看得见。`j`/`k` 选,`Enter` 跳过去
  (pane 行跳它自己,卡片标题跳你上次在那个 session 里的位置),`r` 刷新,`q` 回列表。
  预览给的是**状态,不是画面** —— 画面列表里已经有了:这个 pane 在哪、在这个状态待了多久、
  上下文多满、属于哪个编队、今天读进去多少。它只读,不会把任何 pane 标成已读。
- `t` 打开 token 消耗页:上面是窗口总览(轮数 + 四类 token 的占比),下面是**能走的会话排行**。
  排行按**读入量**排,并且单独高亮**平均每轮上下文** —— 花费 ≈ 轮数 × 每轮扛的上下文,而
  ~98% 的 token 是重读已有上下文,所以只看轮数会看错谁贵。每一行用的是**会话的名字**而不是
  那串 id:你自己起的、编队给的,都没有就退回它的开场提问。`j`/`k` 走,右边预览把这个会话讲
  完整:它被要求做什么、token 花在哪、分天曲线,还活着就给实时画面,已经结束就给它最后那段
  回复。`Enter` 直接跳到那个会话的 pane;它要是已经关掉了,会告诉你跳不过去、并留在原处。
  `1` / `7` 在今天和近 7 天之间切,`p` 收起预览,`r` 重新统计,`q` 回列表。
- `l` 把光标停在 lead 那一行时,展开它的队员,`j`/`k` 就能在队员之间走;在队员行上按 `h`
  收回去。`h`/`l` 本来就是"上一层/下一层"(session 模式就是 pane 的上一层),在这两种行上
  只是再进一层;其他行上它们的作用一点没变。见下。
...
如果你在用 Claude Code 的编队,picker 会把每个队员的 pane 标上它**真正的名字**
(而不是那串几个 pane 共用的窗口标题),并把编队的摘要放进**它所在那个会话的会话头**里 ——
不再单开一块,因为编队和它被拉起来的那个会话本来就是同一个东西。选中那个会话头,预览分成
两半:上半是完整花名册 + 共享任务表(**包括那些没有 pane、跳不过去的成员**),下半是这个
会话自己那些 pane。

队员那几行会**缩进到 lead 底下**,而且默认是"给你看的,不是给你选的":`j`/`k` 会跨过它们,
它们也没有行号 —— 所以一个编队在列表里只占**一个**落点,而不是每个成员占一个。反正真正
值得跳过去的是 lead:队员就在它同一个 tmux window 里,一个原生的切 pane 快捷键就到。

真想在队员之间移动的时候,在 lead 行上按 `l`:那一个编队的队员变成落点,`j`/`k` 就在它们
之间走,`h` 收回去。这只是"暂时进去一趟",不是一个要你记住的模式 —— 走出这个编队它自己就
收起来了,重建列表(`a` / `f` / `ctrl-x`)或者用数字跳走也一样。要看全部编队用 `f`。

名字用的是 Claude Code 给这个队员分配的**颜色**,所以"这个 pane 属于某个编队"这件事
**一列都不占** —— 名字列是这个列表里唯一会不够用的一列,而在每个成员行前面放一个词,
是拿它五分之一的宽度去重复会话头已经说过一遍的话。颜色不是唯一的线索:队员行的行号栏
是空的、`j`/`k` 会跨过它、它的预览里也用文字写着编队和成员名。
...
把 `CLAUDE_TMUX_TEAM_LABELS` 指向一个 JSON(`{"成员名": "词"}`),就能自己决定**预览里**
每个成员的角色显示哪个词。picker 只负责把它显示出来,不解释它是什么意思。
