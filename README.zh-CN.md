...
- `o` 打开总览:一屏回答"我回来了,现在是什么局面"。**第一行就是结论** —— `5 个有结果等你看`
  或者 `没人等你 · 2 个还在跑`;下面按你真正的提问顺序排:**等你的**(⏸ WAIT 在前,然后是
  没读过的 ✔ DONE,等得最久的在最上面)、**还在跑的**、**编队**(花名册 + 任务计数 + 每个
  成员自己那个 pane 的状态);最后是 5h 额度、7 天窗口、今日 token 和近 14 天曲线。
  只渲染一次就等着,`r` 手动刷新,`q` 回列表 —— 它只读,不会把任何 pane 标成已读。
